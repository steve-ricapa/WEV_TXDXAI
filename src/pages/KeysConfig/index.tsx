...
const KeysConfig: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // error estará disponible en estado
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {credentialOptions.map(option => (
            <div key={option.type} className="bg-white border border-gray-200 rounded-lg shadow hover:shadow-md p-6 transition text-center">
              <h2 className="text-xl font-semibold mb-2" style={{ color: option.type === 'MERAKI' ? '#1E40AF' : option.type === 'WAZUH' ? '#047857' : '#B45309' }}>
                {option.label}
              </h2>
              ...
              <button
                ...
                className={`mt-4 w-full py-2 rounded text-white ${
                  option.type === 'MERAKI' ? 'bg-blue-600 hover:bg-blue-700' :
                  option.type === 'WAZUH'  ? 'bg-green-600 hover:bg-green-700' :
